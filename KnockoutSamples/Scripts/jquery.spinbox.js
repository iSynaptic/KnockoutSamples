...
(function ($) {
	...
	;
		...
		.addClass(options.spinboxClass);
			...
			if (options.mousewheel) {
				...
				$(this).bind("mousewheel.spinbox DOMMouseScroll.spinbox", function (e) {
					...
					if (e.detail || e.wheelDelta) {
						...
						// jquery is wrapping the event
						adjustValue.apply(this, [e, step, e.originalEvent.wheelDelta >= 120 ? 1 : -1]);
					}
			...
			function adjustValue(e, step, direction) {
				...
				// [CMM] one place I listen for this is in ManageVM.js
				$box.trigger('afterSpin', data);

				// custom VMware Go code, added 10/26/2011
				try {
					updatePrice();
				}
				catch (e) { }
	...
	$.fn.spinbox.defaults = {
		...
		// Array of regular expressions and/or char codes to restrict key input. Default: 0-9, Tab, Enter, Backspace, Delete, PageUp, PageDown, Left, Up, Right, Down, Minus, Comma, Dot.
		// [CMM] added numpad keycodes b/c the keycode is wrong on the keydown event but neither can we can't use the keypress event b/c the arrow keys do not throw it.
		keys: [/[0-9]/, 9, 13, 8, 46, 33, 34, 37, 38, 39, 40, 109, 188, 190, 96, 97, 98, 99, 100, 101, 102, 103, 104, 105],
